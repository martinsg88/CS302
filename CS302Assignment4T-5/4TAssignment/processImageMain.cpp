...
int connectedComponents(ImageType&, ImageType&, RegionList<ItemType>&, int);

//void findComponentBFS(ImageType&, ImageType&);
template<class ItemType>
void findComponentBFS(ImageType&, ImageType&, RegionList<ItemType>&);

//void findComponentDFS(ImageType&, ImageType&);
template<class ItemType>
void findComponentDFS(ImageType&, ImageType&, RegionList<ItemType>&);
...
void countLabelRegions(int N, int M, int Q, bool type, ImageType& image)
{
	...
	coincount = connectedComponents(image, outputImage, regionList, choice);
}
//------------------------------------------------------------------------------
...
int connectedComponents(ImageType& image, ImageType& outputImage,
											RegionList<ItemType>& regionList, int choice)
{
	...
	if(choice == 2)
	{
		findComponentDFS(image, outputImage, regionList);
	}
...
{
	int vall, vali, valo, size=0;//size is pixel counter
			...
			for(int j = 1; j < M-1; j++)
			{
				...
				if(vali == 255)
				{
					...
					while(!queue.isEmpty())
					{
						...
						//pixeltype.insert(popi,popj)
						//pixellist.InsertItem(pixeltype);
						...
						for(int k = popi-1; k<popi+2; k++)
						{
							for(int l = popj-1; l<popj+2; l++)
							{
								...
								if(vali == 255)
								{
									sumX += popi;
						         sumY += popj;
									size++;//count pixels in a coin image
									queue.enqueue(k,l);
							...
							sumX = sumX/size;//compute X centroid
							sumY = sumY/size;//compute Y centroid

							//compare size with train.cpp and set coinID
