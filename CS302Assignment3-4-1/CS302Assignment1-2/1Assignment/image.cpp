...
ImageType::ImageType(int tmpN, int tmpM, int tmpQ)
{
 ...
 for(int i = 0; i < N; i++)
	{
		pixelValue[i] = new int[M];//row elements for the "col" of pointers
			{
				for(int j = 0; j < M; j++)
					{			
						pixelValue[i][j] = 0;
...
ImageType ImageType::enlargeImage(int e, ImageType &object)
{
	...
	for(i = 0; i< N*e; i++)
	{
		...
		{	//going through every element of object
			...
			//putting it into temp 2, "i/e" & N*e" are walking through and
			//putting it in like Prof. Bebis's example
		}		
	}
return (temp2);
}
...
ImageType ImageType::translateImage(int t, ImageType &object)
{
	...
	for(i = 0; i < N; i++)
	{
		for(j = 0; j < M; j++)
		{
			temp.pixelValue[i][j]=object.pixelValue[i][j];//assign obj to temp
		}
	}
