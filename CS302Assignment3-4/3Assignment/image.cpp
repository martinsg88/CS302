...
//------------------------------------------------------------------------------
ImageType ImageType::displayHistogram(ImageType& object)
{
...
void ImageType::dilate(ImageType& object)
{
 for(int i = 1; i<N-1; i++)
  ...
  for(int j = 1; j<M-1; j++)
	{
	if(object.pixelValue[i-1][j-1] == 255 && object.pixelValue[i-1][j]  == 255 &&
		object.pixelValue[i-1][j+1] == 255 && object.pixelValue[i][j-1]  == 255 &&
		object.pixelValue[i][j+1]   == 255 && object.pixelValue[i+1][j-1]== 255 &&
		object.pixelValue[i+1][j]   == 255 && object.pixelValue[i+1][j+1]== 255)
			object.setPixelVal(i, j, 255);

		else
			pixelValue[i][j] = pixelValue[i][j];
		}
	}
}
//------------------------------------------------------------------------------
void ImageType::erode(ImageType& object)
{
 for(int i = 1; i<N-1; i++)
	...
	for(int j = 1; j<M-1; j++)
	 {
	 if(object.pixelValue[i-1][j-1] == 0 && object.pixelValue[i-1][j] == 0 && 
		 object.pixelValue[i-1][j+1] == 0 && object.pixelValue[i][j-1] == 0 &&
		 object.pixelValue[i][j+1] == 0 && object.pixelValue[i+1][j-1] == 0 &&
		 object.pixelValue[i+1][j] == 0 && object.pixelValue[i+1][j+1] == 0)
			pixelValue[i][j] = 0;
	 //else
		//pixelValue[i][j] = 0;
	 }
